...
def analyze_graph_potential_impl(db: SQLiteMemoryDatabase) -> ToolResponse:
    ...
    try:
        ...
        return cast(ToolResponse, {
            'success': False,
            'error': f"Failed to analyze graph potential: {str(e)}"
        })
    ...
    try:
        ...
        if node_types:
            filtered_nodes = [
                ...
                if node['nodeType'] in node_types
            ]
    ...
    try:
        ...
        if len(isolated_nodes) > total_nodes * 0.3:
            insights['recommendations'].append(
                f"High isolation: {len(isolated_nodes)} nodes have no connections. Consider adding semantic relationships or reviewing data organization."
            )
        ...
        return {
            ...
            'graph_health_score': min(10, max(1, int(connectivity * 5 + (1 - len(isolated_nodes)/total_nodes) * 5)))
        }
