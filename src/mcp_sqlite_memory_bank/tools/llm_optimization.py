    ...
    try:
        ...
        with db.engine.connect() as conn:
            ...
            sample_result = conn.execute(
                text(f"SELECT id, {', '.join(content_columns)} FROM `{table_name}` LIMIT 20")
            )
        ...
        data_summary = "\\n".join([
            f"ID {row['id']}: " + " | ".join([str(row[col]) for col in content_columns if row.get(col)])
            for row in sample_data[:10]
        ])
        
        # This would use MCP sampling (requires client support)
        # For now, return structured analysis format that could be enhanced with sampling
        analysis_prompt = f"""
...
def smart_archiving_policy(
    ...
    try:
        ...
        with db.engine.connect() as conn:
            ...
            temporal_result = conn.execute(text(f"""
                SELECT 
                ...
                LIMIT 30
            """))
            temporal_data = [dict(zip(['date', 'records', 'earliest', 'latest'], row)) 
            ...
            if content_columns:
                content_result = conn.execute(text(f"""
                    SELECT timestamp, {', '.join(content_columns[:3])}
                    FROM `{table_name}`
                    ORDER BY timestamp DESC
                    LIMIT 10
                """))
                content_sample = [dict(zip(['timestamp'] + content_columns[:3], row)) 
