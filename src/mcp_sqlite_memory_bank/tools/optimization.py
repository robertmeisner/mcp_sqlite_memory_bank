    ...
    try:
        ...
        if not rows:
            return cast(ToolResponse, {
                "success": True,
                "duplicates": [],
                "stats": {"total_rows": 0, "duplicate_groups": 0, "potential_savings": 0}
            })
...
def optimize_memory_bank(
    ...
    try:
            ...
            if "timestamp" in columns:
                ...
                if old_rows > 0:
                    optimizations_performed.append({
                        ...
                        "action": f"Archive old entries" if not dry_run else f"Would archive {old_rows} old entries",
            ...
            if "embedding" in columns:
                ...
                if embedding_rows > 0:
                    sample_embedding_result = conn.execute(
                        text(f"SELECT LENGTH(embedding) FROM `{table_name}` WHERE embedding IS NOT NULL LIMIT 1")
                    )
        ...
        if potential_savings["rows_removed"] > 0:
            ...
            potential_savings["performance_improvement"] = min(50, int((potential_savings["rows_removed"] / total_rows) * 100) if total_rows > 0 else 0)
...
def _generate_cleanup_recommendations(duplicate_groups: List[Dict[str, Any]]) -> List[str]:
