...
async def test_numeric_types_handling(temp_db):
    ...
    async with Client(smb.app) as client:
        ...
        await client.call_tool("create_row", {
            "table_name": "numerics",
            "data": {
                "int_val": 2147483647,  # Max 32-bit integer
                "real_val": 1.23456789,
...
async def test_null_and_optional_columns(temp_db):
    ...
    async with Client(smb.app) as client:
        ...
        bad_create = await client.call_tool("create_row", {
            "table_name": "with_nulls",
            "data": {"optional_text": "will fail"}
        })
...
async def test_batch_operations(temp_db):
    ...
    async with Client(smb.app) as client:
        ...
        rows = await client.call_tool("read_rows", {
            "table_name": "batch_test",
            "where": {"value": 3}
        })
        rows_out = extract_result(rows)
        ...
        update = await client.call_tool("update_rows", {
            "table_name": "batch_test",
            "data": {"value": 100},
            "where": {"id": 1}
        })
        ...
        # Verify the update
        read = await client.call_tool("read_rows", {
            "table_name": "batch_test",
            "where": {"id": 1}
        })
        ...
        delete = await client.call_tool("delete_rows", {
            "table_name": "batch_test",
            "where": {"id": 1}
        })
        ...
        # Verify deletion
        remaining = await client.call_tool("read_rows", {
            "table_name": "batch_test",
            "where": {"id": 1}
        })
        ...
        bad_update = await client.call_tool("update_rows", {
            "table_name": "batch_test",
            "data": {"nonexistent": 1},
            "where": {"id": 2}
        })
