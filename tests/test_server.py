    ...
    def test_delete_rows(self):
        ...
        create = smb._create_row_impl(
            table_name='users', data={
                'name': 'Dave', 'age': 40})
        user_id = create['id']
        del_res = smb._delete_rows_impl(
            table_name='users', where={'id': user_id})
...
class TestKnowledgeGraph:
    ...
    def test_knowledge_graph_crud(self):
        ...
        n2 = smb._create_row_impl(
            table_name='nodes', data={
                "label": "Company"})
        ...
        edges = smb._read_rows_impl(
            table_name='edges', where={
                "type": "works_at"})
        assert edges['success']
        assert any(edge['source'] == n1['id'] and edge['target']
                   == n2['id'] for edge in edges['rows'])
        upd = smb._update_rows_impl(
            table_name='nodes', data={
                "label": "Human"}, where={
                "id": n1['id']})
        assert upd['success']
        del_edge = smb._delete_rows_impl(
            table_name='edges', where={"id": e1['id']})
        assert del_edge['success']
        edges2 = smb._read_rows_impl(
            table_name='edges', where={
                "id": e1['id']})
